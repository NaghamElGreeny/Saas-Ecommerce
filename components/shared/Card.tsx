...
function Card({ item, offer,width}: any) {
    ...
    return (
        <div
            onClick={handlePress}
            style={{ width }}
            className={`flex-shrink-0 h-[509px] w-[302px] pt-2.5 pb-6 px-4 bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow cursor-pointer`}
        >
            ...
            <div className="item-desc flex flex-col min-h-[134px] justify-around shrink-1">
                ...
                <div className="flex flex-1 justify-between items-center mt-2">
                    <p className="text-2xl font-bold">{item.price.price} <span className='text-lg'>{item.price.currency}</span> </p>
