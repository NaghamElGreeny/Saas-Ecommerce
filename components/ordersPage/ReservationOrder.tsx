...
import { getReservation } from "@/services/ClientApiHandler";
...
const Reservation: React.FC<ReservationProps> = ({ slugg }) => {
  ...
  useEffect(() => {
    const fetchReservation = async () => {
      try {
        ...
        const data = await getReservation(slugg);
  ...
  if (!reservation) return  <div className="loading w-full flex min-h-screen items-center justify-center">
    ...
    <div className="containerr my-12 grid min-h-[70vh] w-full grid-cols-1 gap-y-4 px-10 lg:grid-cols-2">
      <div className="reservation-2 lg:reservation-1">
        <ReservationDetails
         order={reservation}
        />
              reservation detailsss
      </div>
      <div className="reservation-1 flex flex-col items-center rounded-2xl bg-white px-2 lg:reservation-2">
        <div className="reservationid-status flex w-full px-6 pt-5">
          <h2 className="text-[32px] font-bold">
            Booking Id - {reservation.id}
          </h2>
        </div>
              <div className="totalamount flex flex-col w-full px-6 pt-5">
                  ...
                  <div className="mt-3 flex flex-wrap items-center justify-between rounded-xl border border-[black] px-5 py-3 font-semibold">
                      ...
                      <p>{reservation.total_amount} <span className="font-thin">{ reservation.currency}</span></p>
                  </div>     
 </div>
      </div>
    </div>
